...
public class GenerarNotaCreditoDIAN {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String nitOfeSinDv = NIT_OFE_SIN_DV;
        ...
        Element creditNote = doc.createElement("CreditNote");
        // ===== Declarar todos los namespaces requeridos por DIAN =====
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns", NS_CREDIT_NOTE);
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:cac", NS_CAC);
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:cbc", NS_CBC);
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:ext", NS_EXT);
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:sts", NS_STS);
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:ds", NS_DS);
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:xsi", NS_XSI);
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:xades", "http://uri.etsi.org/01903/v1.3.2#");
        creditNote.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:xades141", "http://uri.etsi.org/01903/v1.4.1#");

        // schemaLocation
        creditNote.setAttributeNS(NS_XSI, "xsi:schemaLocation",
                NS_CREDIT_NOTE + " http://docs.oasis-open.org/ubl/os-UBL-2.1/xsd/maindoc/UBL-Invoice-2.1.xsd");
        ...
        // ===== Encabezados UBL =====
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:UBLVersionID", "UBL 2.1"));
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:CustomizationID", "20"));
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:ProfileID", "DIAN 2.1: Nota Crédito de Factura Electrónica de Venta"));
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:ProfileExecutionID", TIPO_AMBIENTE));
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:ID", numFac));

        Element uuid = textEl(doc, NS_CBC, "cbc:UUID", cude);
        uuid.setAttribute("schemeID", "2");
        uuid.setAttribute("schemeName", "CUFE-SHA384");
        creditNote.appendChild(uuid);

        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:IssueDate", fecFac));
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:IssueTime", horFac));
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:CreditNoteTypeCode", "91"));
        ...
        creditNote.appendChild(textEl(doc, NS_CBC, "cbc:LineCountNumeric", "1"));
        ...
        // Parties
        creditNote.appendChild(buildSupplier(doc, nitOfeSinDv, dvOfe));
        creditNote.appendChild(buildCustomer(doc, numAdqSinDv, dvAdq, adqName));

        // PaymentMeans
        Element payMeans = el(doc, NS_CAC, "cac:PaymentMeans");
        creditNote.appendChild(payMeans);
        payMeans.appendChild(textEl(doc, NS_CBC, "cbc:ID", "1"));
        payMeans.appendChild(textEl(doc, NS_CBC, "cbc:PaymentMeansCode", "10"));
        ...
        creditNote.appendChild(legalTotal);
        ...
        // Línea
        Element line = el(doc, NS_CAC, "cac:CreditNoteLine");
        creditNote.appendChild(line);
        line.appendChild(textEl(doc, NS_CBC, "cbc:ID", "1"));

        Element qty = textEl(doc, NS_CBC, "cbc:CreditedQuantity", "1.000000");
    ...
    // ==================== CUDE helpers ====================
    private static String buildCudeBase(
